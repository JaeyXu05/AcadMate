...
> 2026-09-03 已完成第二轮人工复核。`data/ustc_mentor_rag.json` 已升级为「972 导师 / 1969 证据」，
> 完成离线清洗、论文指标拆分、作者审核隔离和官方主页导师身份恢复，并保持
> 与 A 后端检索、B 云图、D 前端三者 `candidate_id` 一致。

## 2026-09-03 完成结果

- 方向：668/972；确定模板/论文标题垃圾 0，另有 7 条较长但语义合理的研究描述保留为复核提示。
- 论文：259 位含已确认代表作；367 条 confirmed、0 条 pending_review 论文证据。191 个歧义平台命中已人工裁决（41 保留、150 排除）。
- 身份：721 条结构字段核验 + 251 条官方主页强语句或人工官网复核；陈立锋、李微雪 2 条证据不足未进仓。
- bio/email：758/193；bio 不再以整页前 600 字作为伪覆盖。
- `verify_rag.py` A–G 全 PASS，轻量环境下 D/F 不再跳过。
